...
impl ToolRegistry {
        ...
        for (index, tool) in resolved_tools.iter().enumerate() {
            match tool {
                ...
                ResponsesTool::ToolSearch(p) => {
                    tracing::debug!(execution = ?p.execution, "tool_search skipped in dispatch registry");
                }
    ...
    #[must_use]
    pub(crate) fn can_restore_tool_search_fallback(&self) -> bool {
...
mod tests {
    ...
    #[tokio::test]
    async fn tool_search_fallback_respects_real_and_loaded_function_ownership() {
        let mut tools: Vec<ResponsesTool> = serde_json::from_value(serde_json::json!([
            {"type": "function", "name": "tool_search", "description": "real function"},
            {
                "type": "tool_search",
                "execution": "client",
        ...
        let mut real_call = vec![OutputItem::FunctionCall(FunctionToolCall {
            id: "fc_real".to_owned(),
            call_id: "call_real".to_owned(),
            name: tool_search::TOOL_SEARCH_NAME.to_owned(),
            namespace: None,
            arguments: "{}".to_owned(),
            status: MessageStatus::Completed,
        })];
        registry.restore_final_payload_output(&mut real_call);
        assert!(matches!(real_call[0], OutputItem::FunctionCall(_)));

        let mut search_only_tools: Vec<ResponsesTool> = serde_json::from_value(serde_json::json!([{
            "type": "tool_search",
            "execution": "client",
            "description": "search deferred tools",
            "parameters": {"type": "object"}
        }]))
        .expect("valid search declaration");
        let mut search_registry = ToolRegistry::build_with_handlers(&mut search_only_tools, &mut executors)
            .await
            .expect("valid registry");
    ...
    #[tokio::test]
    async fn hosted_tool_search_does_not_enable_client_fallback_restoration() {
        for declaration in [
            serde_json::json!({"type": "tool_search"}),
            serde_json::json!({"type": "tool_search", "execution": "server"}),
        ] {
            let mut tools: Vec<ResponsesTool> =
                serde_json::from_value(serde_json::json!([declaration])).expect("valid hosted declaration");
            let mut executors = GatewayExecutors::default();
            let registry = ToolRegistry::build_with_handlers(&mut tools, &mut executors)
                .await
                .expect("valid registry");
    ...
    #[tokio::test]
    async fn namespaced_member_named_tool_search_does_not_own_unqualified_fallback() {
        let mut tools: Vec<ResponsesTool> = serde_json::from_value(serde_json::json!([
            {
                "type": "namespace",
                "name": "fixture",
        ...
        assert!(registry.lookup("agentic_ns__fixture__tool_search").is_some());
        assert!(registry.can_restore_tool_search_fallback());
    }
}
