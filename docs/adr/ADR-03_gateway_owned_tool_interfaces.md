...
should see a stable tool contract even if the gateway changes the backend implementation later.
...
There is also a third case: a stable gateway-owned tool may use a tool-shaped backend internally. For example,
`web_search` may use an MCP server such as Exa or Fetch as an implementation backend, while still exposing only the
stable `web_search` contract to the model.

Without explicit interface categories, tool implementations tend to collapse several concerns together:
...
2. model-facing vs backend-facing tool contracts
3. static built-in tools
4. dynamic model-facing toolsets
5. backend adapters behind stable built-ins
6. shared runtime clients for hosted tool backends
...
flowchart LR
    ...
    Static["Static built-in tool contract"]
...
|---|--------------------|---------------------|
| C1 | Static gateway built-ins | Which tools should have stable gateway-owned model-facing contracts? |
| C2 | Dynamic model-facing toolsets | Which tools should expose discovered external tool inventory directly to the model? |
| C3 | Backend adapters | Which stable built-ins need interchangeable MCP, HTTP, local-service, or provider-SDK backends? |
| C4 | Shared runtime clients | Which backends need lifecycle isolation, process sharing, or centralized inventory/status? |

## Interface categories

### 1. Static gateway built-in tool

A static gateway built-in is a stable, model-facing tool owned by the gateway.
...
- backend implementation may change without changing the model-facing contract
...
### 3. Backend adapter behind a built-in

A backend adapter is not model-facing. It is an implementation interface used by a static built-in.

Use this interface when:

- the model-facing contract should stay stable
- different providers can satisfy the same built-in action
- a backend may be MCP, HTTP, local code, a database, or another service
- the gateway needs provider replacement, fallback, or profile selection

Examples:

- `web_search` using an Exa MCP-backed search adapter
- `web_search` using DuckDuckGo for search and Fetch MCP for `open_page`

The contract should include:

- action name supported by the adapter
- runtime requirements, such as required hosted MCP server labels
...
A shared runtime client isolates long-lived backend lifecycle from request handling.

Use this interface when:

- backend sessions are expensive or long-lived
- multiple gateway workers need shared access
- helper processes or subprocesses must be supervised
- tool inventory should be cached or refreshed centrally
- startup failures need a single inspectable status

Examples:

- hosted MCP runtime client
- code interpreter service client, if the interpreter runs as a separate helper service
...
how to sandbox and execute code. The open question is how much of the runtime/service boundary must be standardized in
the first implementation.

For `web_search` or `file_search`, the distinction matters more. A provider may expose an MCP tool, an HTTP API, an
SDK, a vector database, an object store, or a local adapter. Exposing that backend directly would make provider choice
part of the public/model-facing contract. Wrapping it behind a static built-in may keep the interface stable and let the
gateway own action semantics, output shape, and include behavior, but it also means the gateway must define and maintain
its own abstraction instead of reusing the backend schema directly.
...
- use a static built-in when the gateway owns the product abstraction
- use a dynamic toolset when the discovered backend inventory is the product abstraction
- use backend adapters when a stable built-in needs interchangeable implementation providers
- use runtime clients when backend lifecycle should not live inside request handlers

## Consequences to evaluate

If the project adopts these categories, the likely consequences are:

- adding a new stable built-in would require a public declaration, model-facing schema, executor, typed result, and
  response adapter
...
- MCP could be used either as a model-facing dynamic toolset or as a hidden backend adapter
- runtime/session lifecycle could be centralized without forcing every tool to know how that lifecycle works
...
- error handling policy would differ by interface type: static built-ins can provide product-specific errors, while
  dynamic toolsets need robust wrapping around external volatility

These consequences need review before acceptance. In particular, the committee should decide which categories are
needed for the MVP, which can be deferred, and whether one generic interface should cover all categories or whether the
