...
Interactive clients already own that lifecycle:

- Codex maintains its own MCP connections and resource-facing client behavior outside the OpenAI model wire. Its
  gateway-facing model calls are converted back into the client lifecycle before the next inference request.
- [Claude Code](https://code.claude.com/docs/en/mcp#use-mcp-resources) exposes MCP resources through `@` references,
  fetches selected resources, and includes them as attachments.

The previous Codex-compatible function bridge accepted a client-declared function named `read_mcp_resource`, decoded
custom metadata containing MCP server URLs, and translated the function call into `resources/read`. That path was not
part of the OpenAI Responses `type: "mcp"` contract and has been removed.
...
server can expose a real `read_resource` tool through `tools/list`; that is already compatible with the gateway because
the gateway treats it as an ordinary MCP tool and invokes it through `tools/call`.
...
no client and exists only for `ResponsesTool::to_function_tools()` normalization. There is no operation enum because
the handler no longer switches between `tools/call` and `resources/read`.
