...
The feature-gated Eryx test requires a runtime prepared by `scripts/setup-eryx-runtime.sh` (or selected at build time with `ERYX_RUNTIME_CWASM`) and a fresh private `TMPDIR`. It executes a successful program, a Python exception, filesystem isolation probes, and output overflow through the real `eryx::Sandbox`. The setup-script test itself uses a fake precompiler to verify version matching and the `cargo install`/`cargo binstall` branches without downloading a runtime.
